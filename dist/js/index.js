...
function fetchPercHealthFA(urlfa,itemnames,countyid,periodid){
    ...
    error: function (request, status, error, message) {
            ...
            $('.percent_healthfa').addClass('hidden');
            $('malaria_commodity_table.t_one').addClass('hidden');
            console.log('MainDash: error fetching json. Error:- '+error + ' & Status:- '+status);
            $('.t_one_state').html('<div class ="alert alert-danger"><strong>Data Error: '+message+'</strong><br/>Failed to load this data. Please <a href="#" class="btn btn-xs btn-primary btn-rounded" onclick="window.location.reload(true)">refresh</a> this page to retry</div>');
        }
    });
}
